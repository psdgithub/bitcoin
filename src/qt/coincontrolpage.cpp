...
void CoinControlPage::UpdateTable()
{
    ...
    std::set< std::set<CTxDestination> > groupings = pwalletMain->GetAddressGroupings();
    std::vector< PAIRTYPE(int64, std::set<CTxDestination>) > nonZeroGroupings;

    BOOST_FOREACH(std::set<CTxDestination> addresses, groupings)
    {
        int64 balance = 0;
    ...
    bool first = true;
    BOOST_FOREACH(PAIRTYPE(int64, std::set<CTxDestination>) grouping, nonZeroGroupings)
    {
        int64 group_balance = grouping.first;
        std::set<CTxDestination> addresses = grouping.second;
        if (first)
            first = false;
        else
            ui->table->insertRow(0);

        std::vector<CTxDestination> sortedAddresses(addresses.begin(), addresses.end());
        ...
        BOOST_FOREACH(CTxDestination address, sortedAddresses)
        {
            int64 balance = balances[address];
            remaining--;

            ui->table->insertRow(0);
            ui->table->setItem(0, 0, new QTableWidgetItem(QString::fromStdString(CBitcoinAddress(address).ToString())));
...

            {
                ...
                {
                    QString strLabel = QString::fromStdString(pwalletMain->mapAddressBook.find(address)->second);
