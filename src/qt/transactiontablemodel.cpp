...
public:
    ...
    void updateWallet(const uint256 &hash, int status)
    {
        OutputDebugStringF("updateWallet %s %i\n", hash.ToString().c_str(), status);
        {
            ...
            if(status == ES_UPDATED)
            {
                if(showTransaction && !inModel)
                    status = ES_NEW; /* Not in model, but want to show, treat as new */
                if(!showTransaction && inModel)
                    status = ES_DELETED; /* In model, but want to hide, treat as deleted */
            }
            ...
            case ES_NEW:
                if(inModel)
                {
                    OutputDebugStringF("Warning: updateWallet: Got ES_NEW, but transaction is already in model\n");
                    break;
                }
                if(!inWallet)
                {
                    OutputDebugStringF("Warning: updateWallet: Got ES_NEW, but transaction is not in wallet\n");
                    break;
                }
                if(showTransaction)
                {
            ...
            case ES_DELETED:
                if(!inModel)
                {
                    OutputDebugStringF("Warning: updateWallet: Got ES_DELETED, but transaction is not in model\n");
                    break;
                }
