...
bool Solver(const CScript& scriptPubKey, uint256 hash, int nHashType, CScript& scriptSigRet)
{
    ...
    txntype ttype = TX_NONSTANDARD;
    vector<vector<unsigned char> > vSolutions;
    if (!Solver(scriptPubKey, ttype, vSolutions))
        return false;
    ...
    {
        // No support for spending TX_SCRIPTHASH or TX_MULTISIG
        // transactions to you-- supporting those requires
        // more extensive changes to the wallet. This isn't
        // a problem, because you won't get SCRIPTHASH or MULTISIG
        // transactions sent to you unless your wallet DOES fully
        // support them.
        return false;
    }
...
bool ExtractPubKey(const CScript& scriptPubKey, bool fMineOnly, vector<unsigned char>& vchPubKeyRet)
{
    ...
    CRITICAL_BLOCK(cs_mapKeys)
    {
        ...
        else if (ttype == TX_PUBKEYHASH)
        {
            ...
            map<uint160, valtype>::iterator mi = mapPubKeys.find(hash);
            if (mi == mapPubKeys.end())
                return false;
            vchPubKey = (*mi).second;
        }
