...
bool CBlock::AcceptBlock(bool fWriteToDisk)
{
    ...
    if (!Checkpoints::CheckBlock(nHeight, hash))
        return reject("checkpoint-mismatch", DoS(100, error("AcceptBlock() : rejected by checkpoint lockin at %d", nHeight)));
    ...
    if (nVersion >= 2)
    {
            ...
            (fTestNet && CBlockIndex::IsSuperMajority(2, pindexPrev, 51, 100)))
        {
            ...
            if (!std::equal(expect.begin(), expect.end(), vtx[0].vin[0].scriptSig.begin()))
                return reject("bad-cb-height", error("AcceptBlock() : block height mismatch in coinbase"));
        }
    }
