...
public:
    ...
    bool CreateTransaction(const std::vector<std::pair<CScript, int64_t> >& vecSend,
                           CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl *coinControl=NULL, const int64_t nMaxFee=0x7fffffff);
    bool CreateTransaction(CScript scriptPubKey, int64_t nValue,
                           CWalletTx& wtxNew, CReserveKey& reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl *coinControl=NULL, const int64_t nMaxFee=0x7fffffff);
    bool CommitTransaction(CWalletTx& wtxNew, CReserveKey& reservekey);
    std::string SendMoney(CScript scriptPubKey, int64_t nValue, CWalletTx& wtxNew, bool fAskFee=false, const int64_t nMaxFee=0x7fffffff);
    std::string SendMoneyToDestination(const CTxDestination &address, int64_t nValue, CWalletTx& wtxNew, bool fAskFee=false, const int64_t nMaxFee=0x7fffffff);
    ...
    int64_t GetAddressTally(const CTxDestination& dest, int nMinDepth);
    std::set<CTxDestination> GetAccountAddresses(std::string strAccount) const;
    void ListAccountCreditDebit(const std::string& strAccount, std::list<CAccountingEntry>& entries)
    {
        CWalletDB(strWalletFile).ListAccountCreditDebit(strAccount, entries);
    }
    bool MoveBalance(const std::string& strFrom, const std::string& strTo, const int64_t nAmount, const std::string& strComment);
