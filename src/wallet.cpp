        ...
        BOOST_FOREACH(const CTxIn& txin, tx.vin)
        {
            ...
            if (mi != mapWallet.end())
            {
                ...
                if (!wtx.IsSpent(txin.prevout.n) && IsMine(wtx.vout[txin.prevout.n]))
                {
                    ...
                    wtx.WriteToDisk();
                    NotifyTransactionChanged(this, txin.prevout.hash, ES_UPDATED);
                }
            }
        }
    }
...
bool CWallet::AddToWallet(const CWalletTx& wtxIn)
{
    uint256 hash = wtxIn.GetHash();
    {
        ...
        NotifyTransactionChanged(this, hash, fInsertedNew ? ES_NEW : ES_UPDATED);
    }
    return true;
}
...
{
    {
        ...
        printf("CommitTransaction:\n%s", wtxNew.ToString().c_str());
        {
            ...
            BOOST_FOREACH(const CTxIn& txin, wtxNew.vin)
            {
                ...
                coin.WriteToDisk();
                NotifyTransactionChanged(this, coin.GetHash(), ES_UPDATED);
            }
...
string CWallet::SendMoneyToBitcoinAddress(const CBitcoinAddress& address, int64 nValue, CWalletTx& wtxNew, bool fAskFee, const int64 nMaxFee)
{
    // Check amount
    if (nValue <= 0)
...
bool CWallet::SetAddressBookName(const CBitcoinAddress& address, const string& strName)
{
    ...
    NotifyAddressBookChanged(this, address.ToString(), strName, (mi == mapAddressBook.end()) ? ES_NEW : ES_UPDATED);
    if (!fFileBacked)
        return false;
...
bool CWallet::DelAddressBookName(const CBitcoinAddress& address)
{
    ...
    NotifyAddressBookChanged(this, address.ToString(), "", ES_DELETED);
    if (!fFileBacked)
        return false;
...
{
    {
        ...
        if (mi != mapWallet.end())
            NotifyTransactionChanged(this, hashTx, ES_UPDATED);
    }
}
